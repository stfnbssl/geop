/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\geop\packages\history\.wizzi\db\index.js.ittf
*/
'use strict';
//
...
function createWizziFactory(globalContext, callback) {
    ...
    wizzi.fsnoaclFactory({
        plugins: {
            ...
            pluginsBaseFolder: path.resolve(__dirname, '..', '..', 'wizzi.plugin.geop', 'dist')
         }, 
...
function executeGenerateModules(modules, callback) {
    async.mapSeries(modules, (module, callback) => {
        ...
        var ittfDocumentUri = path.join(__dirname, module + '.geop.ittf');
        var outputPath = path.join(__dirname, '..', '..', '..', 'docs', 'json', module + '.geop');
        loadWizziModel(ittfDocumentUri, {}, function(err, model) {
            if (err) {
                return callback(err);
            }
            file.write(outputPath + '.json', stringify(model.toJson(), null, 4))
            loadModelAndTransform(ittfDocumentUri, {}, "geop/extended", function(err, model) {
                if (err) {
                    return callback(err);
                }
